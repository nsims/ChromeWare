...
function openRequest(info, tab) {
  ...
  if(parseInt(number, 10) > 0){
    var url = "https://software.enablon.com/Software/go.aspx?u=%2FReferent%2FRqtes&rid=" + number;
...
function openRequirement(info, tab) {
  ...
  var number = selection.replace(/"/g, "");
  if(parseInt(number, 10) > 0){
    var url = "https://software.enablon.com/Software/go.aspx?u=%2FReferent%2FProreq&rid=" + number;
    chrome.tabs.create({ url: url });
  }
}
...
function openTestFile(info, tab) {
  ...
  if(parseInt(number, 10) > 0){
    var url = "https://software.enablon.com/Software/go.aspx?u=%2FReferent%2FFTs&rid=" + number;
    chrome.tabs.create({ url: url });
  }
}
